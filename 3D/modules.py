...
_cosine = load(name="_cosine", sources = ['/hdd/kng/CosineSampler/3D/cosine_sampler_3d_kernel.cu', '/hdd/kng/CosineSampler/3D/cosine_sampler_3d.cpp'])
...
class SmoothSampler(torch.autograd.Function):
    ...
    def forward(ctx, input, grid, padding_mode="zeros", align_corners=True):
        ...
        output = _cosine.forward(input, grid, ctx.offset, padding_mode_enum(padding_mode), align_corners)
    ...
    def backward(ctx, grad_out):
        ...
        d_input, d_grid = SmoothSamplerBackward.apply(input, grid, grad_out.contiguous(), ctx.offset, ctx.padding_mode, ctx.align_corners)
        return d_input, d_grid, None, None

class SmoothSamplerBackward(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, grid, grad_out, offset, padding_mode="zeros", align_corners=True):
        ctx.align_corners = align_corners
        ctx.padding_mode = padding_mode
        ctx.offset = offset
    ...
    def backward(ctx, gOutInput, gOutGrid):
        ...
        gInput, gGrid, ggOut = SmoothSamplerBackwardBackward.apply(input, grid, gOut, gOutInput.contiguous(), gOutGrid.contiguous(), ctx.offset, ctx.padding_mode, ctx.align_corners)
...
class SmoothSamplerBackwardBackward(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, gOutgInput, gOutgGrid, gOutggOut):
        ...
        b_input, _, _ = SmoothSamplerBackwardBackward.apply(input, grid, gOutggOut.contiguous(), torch.ones_like(gOutgInput), gOutGrid, ctx.offset)

        return gInput+b_input, None, ggOut, None, None, None, None, None
...
if __name__ == "__main__":
    ...
    for padding_mode in ["zeros", "border", "reflection"]:
        for align_corners in [True, False]:
            ...
            # out1 = SmoothSampler.apply(input, grid, padding_mode, align_corners, True)
            # # out2 = torch.nn.functional.grid_sample(input, grid, padding_mode=padding_mode, align_corners=align_corners)
            # out3 = grid_sample_temp.grid_sample_3d(input, grid, step='smoothstep', offset=False).unsqueeze(2)
            ...
            start = time.time()
            val2 = SmoothSampler.apply(cells, grid, padding_mode, align_corners)
            end = time.time()
            ...
            start = time.time()
            val = grid_sample_temp.grid_sample_3d(cells, grid, step='cosine', offset=True).unsqueeze(2)
            end = time.time()
